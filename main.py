...
# === КОНФИГУРАЦИЯ ===
BOT_TOKEN = "8228595010:AAFJHE3PBlAdFy7NV_ZmoQbhBwby2zXOUCo"  # Ваш токен бота
...
if not BOT_TOKEN:
    ...
    print("Замените значение BOT_TOKEN на ваш токен бота")
    exit(1)

if not ADMIN_IDS:
    print("ОШИБКА: Не указаны ADMIN_IDS!")
    print("Добавьте хотя бы один ID администратора")
    exit(1)
...
🤖 Я бот для связи со смертным грехом.
📝 Просто напишите мне сообщение, и я передам его ему.

⚡ Он ответит вам когда захочет.
...
• Аудио • Голосовые сообщения • Стикеры (всë кроме текста в разработке)
...
def forward_to_admin(message):
    ...
    try:
        bot.reply_to(message, "✅ Сообщение отправлено смертному греху!")
...
🕒 <b>Время:</b> {time.strftime('%H:%M:%S')}"""
    ...
    if not success_sent:
        print_info(f"Не удалось отправить сообщение ни одному админу!")
...
Используйте /status для проверки активных диалогов""",
    ...
    try:
        ...
        if message.content_type == 'text':
            bot.send_message(
                user_chat_id,
                f"""👨‍💼 <b>ОТВЕТ ОТ СМЕРТНОГО ГРЕХА</b>
